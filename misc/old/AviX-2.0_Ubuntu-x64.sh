...
# Nemo PPA(WebUpd8Team) #TRUSTY #VIVID #WILY(PARTIAL UPDATE NEEDS_FIX)
...
# Simple screen recorder PPA #TRUSTY #VIVID #WILY
...
sudo dpkg -i ~/Downloads/kxstudio-repos_9.2.2~kxstudio1_all.deb;
# Vertex theme repository #TRUSTY #VIVID #WILY
cd ~/Downloads;
wget http://download.opensuse.org/repositories/home:Horst3180/xUbuntu_`lsb_release -r -s`/Release.key; #WILY TMP_FIX
sudo apt-key add - < Release.key;
echo deb http://download.opensuse.org/repositories/home:/Horst3180/xUbuntu_`lsb_release -r -s`/ / | sudo tee /etc/apt/sources.list.d/vertex-theme-`lsb_release -c -s`.list;
...
# VirtualBox repository #TRUSTY #VIVID #WILY
cd ~/Downloads;
wget -q https://www.virtualbox.org/download/oracle_vbox.asc;
sudo apt-key add - < oracle_vbox.asc;
echo deb http://download.virtualbox.org/virtualbox/debian `lsb_release -c -s` contrib | sudo tee /etc/apt/sources.list.d/virtualbox-`lsb_release -c -s`.list;
# Purple-Facebook repository #TRUSTY #VIVID
cd ~/Downloads;
...
sudo apt-get install -y nemo nemo-dropbox nemo-emblems nemo-share folder-color-nemo vertex-theme numix-icon-theme-circle deluge-gtk python-libtorrent parole qasmixer spotify-client libreoffice-style-breeze shutter vinagre p7zip-rar rygel-preferences gtk2-engines-pixbuf:i386 unity-gtk2-module:i386 pyrenamer filezilla diodon liferea syncthing haguichi preload gshare undistract-me pidgin pidgin-extprefs purple-facebook ffmpegthumbnailer;
sudo apt-get install -y skype;
# NEMO TMP_FIX (PARTIAL UPDATE NEEDS FIX)
sudo sed -i 's/vivid/wily/g' /etc/apt/sources.list.d/webupd8team-ubuntu-nemo-wily.list;
sudo apt-get update;
sudo apt-get dist-upgrade -y;
##############################################################################################################################################################################################################################################################
# Install config apps
...
##############################################################################################################################################################################################################################################################
# Install tool apps
sudo apt-get install -y git checkinstall gparted gdebi gufw unetbootin virtualbox-5.0
...
##############################################################################################################################################################################################################################################################
# Install media apps
sudo apt-get install -y acetoneiso gimp gimp-plugin-registry inkscape blender openshot kdenlive krita kde-l10n-ptbr handbrake brasero asunder ffmpeg lame tuxguitar-fluidsynth fluid-soundfont-gm rawtherapee openlp puddletag simplescreenrecorder simplescreenrecorder-lib:i386;
##############################################################################################################################################################################################################################################################
...
sudo apt-get purge -y xcursor-themes nautilus transmission-common totem-common gnome-mahjongg gnome-sudoku gnome-mines aisleriot empathy gnome-screenshot usb-creator-common remmina-common plymouth-theme-ubuntu-logo plymouth-theme-ubuntu-text plymouth-label software-center-aptdaemon-plugins libreoffice-style-human;
# Removed extra apps
sudo apt-get purge -y xfdesktop4-data xfce4-volumed meterbridge qsynth sni-qt:i386 steadyflow kget;
...
# Syncthing-GTK
cd ~/Downloads
wget -c http://ppa.launchpad.net/nilarimogard/webupd8/ubuntu/pool/main/s/syncthing-gtk/syncthing-gtk_0.8.2-1~webupd8~`lsb_release -c -s`0_amd64.deb;
sudo dpkg -i ~/Downloads/syncthing-gtk_0.8.2-1~webupd8~`lsb_release -c -s`0_amd64.deb;
sudo apt-get install -f -y;
...
cd ~/Downloads;
wget -c http://host-rauldipeas.rhcloud.com/repository/data/debian%20apps/pidgin-sendscreenshot-0.8-3-amd64.deb;
sudo dpkg -i ~/Downloads/pidgin-sendscreenshot-0.8.3-amd64.deb;
##############################################################################################################################################################################################################################################################
# Pidgin Skype Web
cd ~/Downloads;
wget -c https://host-rauldipeas.rhcloud.com/repository/data/debian%20apps/skypeweb_1.0-1_amd64.deb;
...
# CGRU Afanasy
cd ~/Downloads;
wget -c http://ufpr.dl.sourceforge.net/project/cgru/2.0.8/cgru.2.0.8.ubuntu14.04.3_amd64.tar.gz;
mkdir -p -v ~/Downloads/Afanasy;
tar -vzxf ~/Downloads/cgru.2.0.8.ubuntu14.04.3_amd64.tar.gz -C ~/Downloads/Afanasy;
...
cd ~/Downloads;
wget -c http://cinelerra.org/2015/downloads/cinelerra-5.0-ubuntu-14.04.1-x86_64-20151221-static.txz;
cp -v ~/Downloads/cinelerra-5.0-ubuntu-14.04.1-x86_64-20151221-static.txz /opt/cinelerra/;
cd /opt/cinelerra;
tar -xJf /opt/cinelerra/cinelerra-5.0-ubuntu-14.04.1-x86_64-20151221-static.txz;
...
# Natron
cd ~/Downloads;
wget -c http://downloads.natron.fr/Linux/releases/x86_64/files/natron_2.0.0.RC6-2_amd64.deb;
sudo dpkg -i ~/Downloads/natron_2.0.0.RC6-2_amd64.deb;
sudo sed -i 's/Icon=Natron/Icon=natronIcon256_linux/g' /usr/share/applications/Natron2.desktop;
##############################################################################################################################################################################################################################################################
# The Foundry Nuke
cd ~/Downloads;
...
sudo rm -R -v ~/Desktop;
sudo rm -R -v ~/.gnome2;
sudo killall -9 Nuke9.0v7-linux-x86-release-64-installer;
...
# REAPER SWS
cd ~/Downloads;
wget -c http://www.sws-extension.org/download/featured/sws-v2.8.3.0-x64-install.exe;
wine ~/Downloads/sws-v2.8.3.0-x64-install.exe /S;
# REAPER codecs
# FFMPEG
cd ~/Downloads;
...
# VST plugins
cd ~/Downloads;
wget -c http://googledrive.com/host/0BxHEhxnC8oRpZFdHdUNoelE1ZHM/plugins-vst.7z;
mkdir -p -v ~/Plugins\ VST;
7z x ~/Downloads/plugins-vst.7z -o/$HOME/Plugins\ VST/;
...
# Plymouth theme
cd ~/Downloads;
wget -c http://host-rauldipeas.rhcloud.com/repository/data/debian%20apps/avix-plymouth_2.0_all.deb;
...
sudo sed -i 's/Exec=kdenlive/Exec=env GTK_DATA_PREFIX="\/opt\/dark-theme\" kdenlive/g' /usr/share/applications/kde4/kdenlive.desktop /usr/share/applications/org.kde.kdenlive.desktop;
...
sudo sed -i 's/Exec=env /Exec=env GTK_DATA_PREFIX="/opt/dark-theme" /g' /usr/share/applications/skype.desktop;
...
cp -v /usr/share/applications/nemo.desktop ~/.local/share/applications/
sed -i 's/Name=Open a New Window/Name=Abrir uma nova janela/g' ~/.local/share/applications/nemo.desktop;
# /usr/share/applications/pyrenamer.desktop
cp -v /usr/share/applications/pyrenamer.desktop ~/.local/share/applications/
sed -i 's/Icon\=\/usr\/share\/pyrenamer\/pyrenamer.png/Icon\=pyrenamer/g' ~/.local/share/applications/pyrenamer.desktop;
# /usr/share/applications/xdiagnose.desktop
cp -v /usr/share/applications/xdiagnose.desktop ~/.local/share/applications/
sed -i 's/Icon=\/usr\/share\/xdiagnose\/icons\/microscope.svg/Icon=xdiagnose/g' ~/.local/share/applications/xdiagnose.desktop;
# /usr/share/applications/webbrowser-app.desktop
cp -v /usr/share/applications/webbrowser.desktop ~/.local/share/applications/
sed -i 's/Icon=\/usr\/share\/webbrowser-app\/webbrowser-app.png/Icon=webbrowser-app/g' ~/.local/share/applications/webbrowser-app.desktop;
...
cp -v /usr/share/applications/tuxguitar.desktop ~/.local/share/applications/
sed -i 's/Name=tuxguitar/Name=TuxGuitar/g' ~/.local/share/applications/tuxguitar.desktop;
...
sudo cp -v ~/Downloads/sparkle-share-indicators/24/* /usr/local/share/icons/Paper/24x24/status/;
sudo cp -v ~/Downloads/sparkle-share-indicators/48/* /usr/local/share/icons/Paper/48x48/status/;
##############################################################################################################################################################################################################################################################
# Set Folder Icons
# Dropbox
mkdir -p -v ~/Dropbox;
...
#  Opções gerais >  Comportamento de Focar e Elevar > Nível Prevenção de Foco = Desligado
